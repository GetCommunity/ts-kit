...
export type ComboboxMultiInputProps<T> = {
  name: string
...
const ComboboxMultiInput = <T,>(props: ComboboxMultiInputProps<T>) => {
  ...
  return (
    <ComboboxUI<T>
      ...
      class={cn(
        ...
        props.error && "placeholder:text-destructive",
        props.class
      )}
      name={props.name}
    ...
    >
      ...
      <ComboboxControl<T>
        ...
        class={cn("w-full min-h-10 h-auto", props.error ? "border-destructive" : "")}
      >
        {(state) => (
          <>
