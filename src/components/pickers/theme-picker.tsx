...
const getLabel = (theme: Theme) =>
  [...BASE_COLORS, ...THEMES].find((t) => t.name === theme)?.label
const getColor = (theme: Theme) =>
  [...BASE_COLORS, ...THEMES].find((t) => t.name === theme)?.color
        ...
        <DropdownMenuContent class="no-scrollbar max-h-96 w-[calc(100svw-var(--spacing)*4)] md:w-52">
          <DropdownMenuRadioGroup
            ...
            onChange={(value) =>
              navigate({
                to: ".",
                search: (prev) => ({ ...prev, theme: value as Theme })
              })
            }
          >
            ...
            >
              <div class="flex items-start gap-2">
                ...
                <span class="flex flex-col justify-start pointer-coarse:gap-1">
                  <span>
                    {
                      BASE_COLORS.find((c) => c.name === location().search.baseColor)
                        ?.label
                    }
                  </span>
