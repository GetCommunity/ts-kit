...
export const Route = createFileRoute("/_website/ui/$slug/docs")({
  ...
  component: () => (
    <Suspense>
      {/* <Suspense fallback={<DocsSkeleton showToggleNav />}> */}
      <RouteComponent />
    </Suspense>
  ),
...
function RouteComponent() {
  ...
  const primitive = () => "kobalte" // search().primitive
  const MDXContent = lazy(() => import(`@/pages/ui/${primitive()}/${doc().slug}.mdx`))

  return (
    <div
      data-slot="ui-docs-layout"
      class={cn(
        "relative flex w-[calc(100svw-var(--spacing)*8)] flex-row md:w-[calc(100svw-var(--spacing)*56)] lg:w-full",
        "overflow-hidden"
      )}
    >
      ...
      <div
        ...
        class={cn(
          "flex h-full grow justify-center overflow-y-auto scroll-smooth",
          "no-scrollbar"
        )}
      >
